...
### 模型選型（實測數據，2026-03-30）

> ⚠️ **重要發現**: Qwen 3.5 系列預設啟用深度思考（`<think>` tags），80%+ token 是隱藏思考，實際可見輸出速度極慢。VRAM 佔用也遠超預估。

| 角色 | 模型 | VRAM (實測) | 磁碟 | Speed (實測) | Hard Gate |
|------|------|------------|------|-------------|-----------|
| **Embedding（常駐）** | BGE-M3 | **~1,500 MB** | 1.2GB | N/A | ✅ |
| **分類/標籤（主力）** | Phi-4 Mini 3.8B | **~3,750 MB** | 2.5GB | **19.7 tok/s** | ⚠️ < 30 但可用 |
| **摘要/翻譯（批次）** | Qwen 3.5 9B | **~13,000 MB** | 6.6GB | **3.8-4.1 tok/s** | ❌ 太慢 |
| **Stretch goal** | Qwen 3.5 35B-A3B | **~13,400 MB** | 23GB | **0.9-1.8 tok/s** | ❌ 不實用 |

**關鍵差異 vs 預估：**
- Qwen 3.5 9B VRAM: 預估 6GB → 實測 **13GB**（thinking mode 的 KV cache 開銷）
- Qwen 3.5 9B speed: 預估 60-80 tok/s → 實測 **3.8-4.1 tok/s**（thinking tokens 佔 80%+）
- Phi-4 Mini VRAM: 預估 2.5GB → 實測 **3.75GB**

**VRAM 並發組合（實測）：**
- Config A: BGE-M3 (1.5GB) + Phi-4 Mini (3.75GB) = **~5.25GB** ✅ 推薦（充裕）
- Config B: BGE-M3 (1.5GB) + Qwen 3.5 9B (13GB) = **~14.5GB** ⚠️ 勉強（可能部分 offload）
- Config C: Qwen 3.5 35B-A3B alone = **~13.4GB**（VRAM 飽和，僅剩 443 MiB）

**磁碟預算：** 4 模型 = ~33GB（491GB free 完全無壓力）

**GPU 共存規則：** ComfyUI、LM Studio、ChatWithRTX 共享 16GB VRAM。跑 Ollama 推理前先用 `/local-status` 檢查 VRAM。Config A (5.25GB) 與輕量 GPU 任務可共存。

**待解決：** Qwen 3.5 thinking mode 可否關閉？若可，速度預期提升 5-10x。替代方案：Qwen 2.5 7B/14B（無 thinking mode）。

### MCP 整合方案（Spike 驗證完成 2026-03-30）

| 方案 | 結論 |
|------|------|
| **OllamaClaude** (Jadael) | **選用 + 已 patch** — 安裝成功、啟動正常、11+4 tools |
| ollama-mcp (rawveg) | 備選 — 如 OllamaClaude 不穩定 |
| Ollama Anthropic API 直連 | 不選 — 是「替換」非「委派」 |
| LiteLLM Proxy | **不選** — 單人單引擎不需要 |

**OllamaClaude Spike 結果：**
- ✅ Windows 安裝 + 啟動正常（stdio mode）
- ✅ 已 patch: DEFAULT_MODEL → `phi4-mini`、POSIX 路徑正規化
- ⚠️ 只有 9 stars、AGPL-3.0、個人專案
- ⚠️ 非串流回應（等完才返回）、timeout 15 分鐘
- ⚠️ 預期回應時間：小任務 20-60s、中任務 60-120s
- 配置：`.claude/settings.local.json`（project-level）
...
- **Phi-4 Mini 作為互動場景的主力模型**（實測修正）: 19.7 tok/s、3.75GB VRAM。Qwen 3.5 9B 因 thinking mode 只有 3.8 tok/s 且佔 13GB VRAM，不適合互動場景。Phi-4 Mini 速度可接受、品質足以覆蓋分類、簡單問答、格式轉換。
  - *Qwen 3.5 9B 降級為批次模型*: 品質優秀但速度太慢（3.8 tok/s），改用於離線摘要和翻譯
  - *待驗證*: Qwen 3.5 能否關閉 thinking mode 提速？或改用 Qwen 2.5 7B（無 thinking）
