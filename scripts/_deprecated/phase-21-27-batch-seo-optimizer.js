...
/**
 * Phase 21-27 SEO Batch Optimizer - yololab.net
 *
 * 處理文章 33000-31600（~400-500 篇）
 * 分 7 個 Phase，各 100 篇，每 Phase 分 10 批，每批 10 篇
 * 並行 API 調用，自動恢復進度
 */
...
// Phase 定義（ID 從高到低）
const PHASES = {
  21: { start: 33000, end: 32901, pages: "~150-159" },
  22: { start: 32900, end: 32801, pages: "~160-169" },
  23: { start: 32800, end: 32701, pages: "~170-179" },
  24: { start: 32700, end: 32601, pages: "~180-189" },
  25: { start: 32600, end: 32501, pages: "~190-199" },
  26: { start: 32500, end: 32401, pages: "~200-209" },
  27: { start: 32400, end: 32301, pages: "~210-219" },
};
...
async function processPhase(phaseNum, token) {
  ...
  console.log(`📄 頁碼範圍: ${phaseConfig.pages}\n`);
...
async function runPhases() {
  console.log("\n🌟 Phase 21-27 大規模 SEO 優化（~700 篇文章）\n");
  ...
  // 執行每個 Phase
  for (const phaseNum of [21, 22, 23, 24, 25, 26, 27]) {
    allResults[phaseNum] = await processPhase(phaseNum, token);

    // Phase 間延遲
    if (phaseNum < 27) {
      console.log(`⏳ Phase ${phaseNum} 完成，準備 Phase ${phaseNum + 1}...`);
  ...
  console.log(
    `\n成功率: ${((totalSuccess / 700) * 100).toFixed(1)}% (${totalSuccess}/700)`
  );
  console.log(`失敗率: ${((totalFailed / 700) * 100).toFixed(1)}% (${totalFailed}/700)\n`);

  // 導出進度文件列表
  const progressFiles = [21, 22, 23, 24, 25, 26, 27].map((p) => getProgressFile(p));
  console.log(`💾 進度文件：`);
