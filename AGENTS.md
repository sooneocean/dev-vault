...
- **Node.js Layer**: Knowledge management, GitHub API integration, Anthropic SDK
  - `.Codex/lib/` — Custom slash commands (proposal engine, etc.)
...
- **Automation & Scripts**: `scripts/` directory
  ...
  - `session_stop_wrapper.sh` — Vault sync hook (runs on Codex exit)
...
### Node.js Development
- **Proposal Engine**: `.Codex/lib/proposal-engine.js` — Custom decision logic (test via `npm run test:proposal`)
- **GitHub API Wrapper**: `.Codex/lib/github-api.js` — PR/issue automation, rate limiting, changelog generation
- **Vault Iteration**: `.Codex/lib/vault-iteration.js` — Syncs knowledge base changes with git
- **Changelog Generator**: `.Codex/lib/changelog-generator.js` — Auto-generates release notes from commits
- **Anthropic SDK**: For Codex API integration (tested in proposal engine)
...
- **Tracked**: `.Codex/commands/` (prompt templates for slash commands)
...
1. **Stop Hooks**: `session_stop_wrapper.sh` runs AFTER Codex exits — no reasoning available
...
- **Proposal engine fails**: Check `.Codex/lib/proposal-engine.js` and test via `npm run test:proposal`
